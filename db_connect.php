...
class db_connect{
	...
	public function getadminpetshop($id_user){
		try{
			$getConn = $this->connect();
			$stmt = $getConn->prepare("SELECT * FROM tblpetshop WHERE id_hewan=?");
			// $stmt->bind_param('s',$no);
	...
	public function getadminrescure($id_barang){
		try{
			$getConn = $this->connect();
			$stmt = $getConn->prepare("SELECT * FROM tblpetshop WHERE id_barang=?");
			// $stmt->bind_param('s',$no);
